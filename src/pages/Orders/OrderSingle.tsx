...
const OrderSingle = () => {
  ...
  const {singleOrder} = useAppSelector((state)=>state.data)
  useEffect(()=>{
    if(id)
  ...
  const handlePaymentStatus= (e:ChangeEvent<HTMLSelectElement>)=>{
    const {value} = e.target
    if(id)
    dispatch(changePaymentStatus(id,value as PaymentStatus))
  }

  const handleDeleteOrder = ()=>{
    if(id )
    dispatch(setDeleteOrder({orderId:id}))
    dispatch(deleteOrder(id as DeleteOrder['orderId']))
  }
      ...
      <div className="py-20 px-4 md:px-6 2xl:px-20 2xl:container 2xl:mx-auto">
        <div className="flex justify-start item-start space-y-5 flex-col">
          ...
          <p className="text-base dark:text-gray-300 font-medium leading-6 text-gray-600">
            {new Date(singleOrder?.createdAt as string).toLocaleString()}
          </p>
        </div>
        <div className="mt-10 flex flex-col xl:flex-row jusitfy-center items-stretch w-full xl:space-x-8 space-y-4 md:space-y-6 xl:space-y-0">
          <div className="flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8">
            <div className="flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full">
              ...
              {singleOrder && singleOrder?.OrderDetails?.map((item)=>(
                ...
                <div className="flex justify-center items-center w-full space-y-4 flex-col border-gray-200 border-b pb-4">
                  <div className="flex justify-between items-center w-full">
                    ...
                    <p className="text-base dark:text-gray-300 leading-4 text-gray-600">
                      {singleOrder?.Payment?.paymentMethod}
                    </p>
                  </div>
                  <div className="flex justify-between items-center w-full">
                    <p className="text-base dark:text-white leading-4 text-gray-800">
                    ...
                    <p className="text-base dark:text-gray-300 leading-4 text-gray-600">
                      {singleOrder?.Payment?.paymentStatus}
                    </p>
                  </div>
                  <div className="flex justify-between items-center w-full">
                    <p className="text-base dark:text-white leading-4 text-gray-800">
                  ...
                  >
                    <div>
                      ...
                      >
                        ...
                        <option value={OrderStatus.Preparing}>Preparation</option>
                    ...
                    <div>
                      ...
                      >
                        <option value={singleOrder?.Payment?.paymentStatus}>{singleOrder?.Payment?.paymentStatus}</option>
